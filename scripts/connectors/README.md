...
- botify.py      identity walk / org / org/project / org/project/analysis (verdict: status, pages done/known, rate, ETA by cadence, then crawl statistics; a RUNNING crawl is absent from the list and present here) / any app.botify.com URL, reduced to its slug path / BQL query (BOTIFY_API_TOKEN)
