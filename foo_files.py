...
# - TODO (2026-09-23, THE ZERO GATE LET ONE THROUGH; read off this compile's router): the stats block reads "1 already-written articles ... at Mac Shadow" with a blank DOM-hydration line, and it is committed. Two causes, both in prompt_foo.py. (1) update_stats_in_place gates on count == 0, and the Mac shadow corpus now holds one post, so the rewrite fires; gate on the shadow config (PIPULATE_BLOGS_CONFIG naming blogs.shadow.json) instead of the count. (2) The cache holds "hydration": "|", and fetch_honeybot_stats's keep-the-last-reading branch trusts any truthy metrics, so the junk survives every failed fetch; run _HONEYBOT_SHAPE over cached values too, in render_honeybot_stat_lines. Then restore the block to the 1,490 / 2026-09-20T17:42Z reading. Gate: a compile on the Mac leaves git status empty.
