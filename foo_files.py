...
# --- START STATS ---
# There are 1,497 already-written articles about this repo at MikeLev.in (Public)
# Velocity: 13 published in the last 7 days
# Markdown negotiated: 2,751 reads (0.22% of all responses)
# DOM hydration: 2973 trapdoor triggers from 15 non-local IPs (top-N sample, self excluded)
# Honeybot telemetry fetched 2026-09-26T18:23Z
# --- END STATS ---
...
# 2026-09-26 dismount THE ACTUATOR IS NOT THE LEDGER (deed 1577 the AFTER; commits 67bdedc8 the cut, cdd7689c the guard, 017b9c96 the move, 2ef5c9d5 the keys, pushed by hand; the conviction 7dd5d832, and the two apply.py commits before it, 72ea9900 and 7f841138, were prose too): apply.py rode the fixed tail of every compile carrying 22 constitutional rules in 365 comment lines, 25,192 bytes, 45 percent of the file, banked there because a patch there was known to land and the first of them said so in its own text. The block left by a gated line-range cut, moved into foo_files.py under § NEXT CONTEXT IS THE WHOLE LIST by a script reading commit 7dd5d832 so not one of its bytes rode the reply, and apply.py now refuses a patch to itself that adds comment lines and changes no code, both arms, vim the only override. Straddles in band: apply.py 906 -> 592 lines and 55,944 -> 33,806 bytes (-314, -22,138 exact); foo_files.py 2965 -> 3333 and 347,586 -> 375,029 (+368, +27,443 exact); ^# 420 -> 67; § 58 -> 59; FINDING DORY apply.py:301 -> foo_files.py:497; ACTUATOR IS NOT THE LEDGER nothing and rg_exit=1 -> 3 and 3; A FORECAST COUNTS LINES apply.py:1 -> foo_files.py:1; the smoke on a temp copy APPLIED apply_exit=0 -> the refusal apply_exit=1 in the compile lane; git log 7dd5d832 atop two prose commits -> cdd7689c, 67bdedc8, 7dd5d832; diffstats -365, +51, +366, +4/-2 as forecast; Ruff clean; Coverage +0 and integrity 25 of 25, so neither gauge reads prose. Misses mine: four absolutes one over (593, 33,807, 3,334, 375,030) and both GO lines with them, because the copies rebuilt from the payload carried a trailing newline the files lack, every delta exact and the Manifest blamed for my byte (DELTA-NOT-ABSOLUTE amended); the smoke's cd $(mktemp -d) rode last and left the prompt in tmp.37QWFQrBrb, THE PROBE THAT ATE ITS SIBLINGS without a subshell; rg printed foo_files.py before apply.py, set-equal. m named the +366 move "Remove obsolete comments and documentation". UNWITNESSED: the whole-file arm's refusal and a comment-plus-code pass on the real file (fixture only), the vim override, the graduation itself. This block reads one line past the reading above; the forget ride's awk is the count that rules.
...
# 2026-09-24 dismount THE POWER CAME BACK (two compiles, deeds 1554 and 1555; commits 90e32356 and ae93040a, pushed by hand): four days dark on Prime and Honeybot, then one diverged checkout, [ahead 1, behind 129], and the operator's "write over everything" answered by one chained line, git fetch origin && git branch stranded-2026-09-24 && git reset --hard origin/main, HEAD at 12533f06 and git pull reading Already up to date. Straddles in band: status [ahead 1, behind 129] -> ## main...origin/main; the stranded list e3a9cd7e -> empty, and e3a9cd7e is the apply.py BANK car origin already holds as 018937f, exactly as the 2026-09-21 TODO the Mac wrote had read it, a TODO the BEFORE payload could not carry because the Mac's four days arrived with the reset; Update refused 0 -> 1 with hook_syntax_ok on both taps and diffstat +17/-2 against blocks of 3 and 18 (net +15 exact, the shared echo line matched); TODO count 0 -> 3 exact, the Mac's push carrying none. Honeybot answered ssh at 00:12Z before the operator finished typing (the stats fetch stamp moved and the markdown count moved by nine), both doors read 200, and the last request logged before the dark was 21/Sep/2026:00:42:57 -0400, loopback Firefox on X11 fetching the trapdoor beacon on dual-model-failover-independent-backoff, INFERRED to be the stream reading that article aloud. The reset's own bytes convicted the 2026-09-23 zero-gate TODO: the Mac's committed stats block read 2 articles at Mac Shadow with a blank hydration line, and Prime's first compile rewrote it to 1,491 / 2,664 / 2970 from 15 / 2026-09-25T00:12Z in the working tree, uncommitted. The emitter named blast as the caboose against apply.py's PUSH IS ASSUMED, a rule that arrived with the reset one compile after it could be read; the operator pushed by hand as it says. blast was Ctrl+C'd before Car 1 with the odometer staged, and the reset discarded it. UNWITNESSED: the refusal branch itself (it fires only on a diverged default-shell entry, which Car 1 removed), ~/repos/nixos at [behind 15] on both taps with its pull unrun, the shadow rsync, the stream. This block reads +1 against its cap of 20; the forget ride's awk is the count that rules.
# 2026-09-22 dismount THE TEN THOUSAND PROJECT CENSUS (commits a0375ae, 3f24ab1, 088a4a0, e6bc309, 4a7b22d, 47315c0, 6420c39, 7a10797; deed foo-047c44dd-34): the Botify API cannot list projects, so the Django admin export became the census. The unfiltered pull is a 502; four guessed filter keys each answered HTTP 500 on production, so no more guessing. category__exact=1 (Web Property) with --fields id,project_links returned 10,887 rows, 814,503 bytes, POST 123.6s against 67.7s for the id-only run (about 5.1 ms per row for project_links), while `time` read 8m3.689s: the gap was the headful cookie harvest (THE WRAPPER IS NOT THE MEASUREMENT, banked in apply.py). A local parse wrote a 10,887-line queue, 0 unparseable, 0 extra slashes. Misses: a run killed at 2m39s because the POST printed nothing while it waited (user 0.567s, so the server was working; fixed with a pre-wait message); six "No blocks found" refusals from a patch using the wrong closing marker and a four-backtick fence; the stats block rewritten to "1 article at Mac Shadow" once the shadow corpus got its first post. UNWITNESSED: the cookie stopwatch; SpeedWorkers membership for any row.
...
"""
# #todo #to-do #earmarks
# - TODO (earmark 2026-09-26): bring isolation-of-concerns to the main NixOS environment: the Pipulate shell as the everyday command line, the Jekyll co-hosting and the sibling-repo removal as a module beside flake.nix rather than more of it; its own article, after the unify-and-back-up rides.
# - TODO (2026-09-26, THE ACTUATOR IS NOT THE LEDGER; the graduation queue): 22 rules, 365 lines, moved verbatim from apply.py (commit 7dd5d832, lines 17-381) sit below § NEXT CONTEXT IS THE WHOLE LIST under a MOVED FROM line. forget graduates them one rule per car, heaviest first, into a § key here and a body in GLOSSARY.md. The ranker will mis-weigh this block: it splits on '# THE ' and '# § ', so a header without THE (ONE TIMELINE, PUSH IS ASSUMED, IGNITION IS A SENTENCE, A PROBE RUNS, A CENSUS CARRIES, A FORECAST COUNTS, SPELL OUT, ADMIN FORMS, NEXT CONTEXT IS PASTE-READY) lumps into its neighbour and a 'THE RULE:' line splits a body in two; pick by reading the block, not the ranker. OPEN, for the operator: the rules that must ride EVERY compile (ONE OPERATOR INTERFACE, PUSH IS ASSUMED, IGNITION IS A SENTENCE, THE HALT BANNER, THE DIVIDER IS NOT OPTIONAL) have one honest every-compile home, the checklist in prompt_foo.py, one line each; that is a prompt_foo.py ride, unridden. RULED 2026-09-26 at the dismount: thirteen of the 22 are rules about the SHAPE OF THE TURN and belong in the checklist as nine lines, families riding their parent (ONE TIMELINE NO PREVIEWS; THE HUMAN IS NOT THE BRANCH PREDICATE; ONE OPERATOR INTERFACE with PUSH IS ASSUMED; IGNITION IS A SENTENCE, half in item 12 already; THE ANSWER IS THE ARTIFACT; NEXT CONTEXT IS PASTE-READY with NEXT CONTEXT IS THE WHOLE LIST; A PROBE RUNS ON THE OPERATOR'S MACHINE, rg never grep; the hand step as one line, SPELL OUT THE TOOL-CALL with THE HALT BANNER and THE FINDING DORY RULE; THE DIVIDER IS NOT OPTIONAL, no closing marker, beside item 5), and nine are convictions about READING AN INSTRUMENT that are GLOSSARY values with a § key each (THE SCREENSHOT IS NOT A LOCATOR, ADMIN FORMS ARE CREDENTIAL SURFACES, THE COLD-START EXIT IS A FALSE GREEN, THE HELP TEXT IS NOT A CENSUS, A CENSUS CARRIES A KNOWN MEMBER, A FORECAST COUNTS LINES NOT PATTERNS, THE PIPELINE ATE THE FALLBACK, THE WRAPPER IS NOT THE MEASUREMENT, THE A11Y OUTLINE DESCRIBES THE HYDRATED DOM). The thirteen graduate as well, bodies to GLOSSARY.md; their checklist line is the key that rides every compile, which is the one thing apply.py was doing that nothing else did. Gate: rg -n '^# THE FINDING DORY RULE' foo_files.py prints nothing and GLOSSARY.md carries the handle.
# - TODO (2026-09-25, the certifications article): the journal entry that rode dedupe's empty folders, blast's tail, the hyper-literal words and the assurance posture into AI_CONTEXT.md names the control catalogues (NIST SP 800-53 CM-2, CM-3, CM-4, SA-10, SI-7; SP 800-218 PS.3, PW.8; ISO/IEC 27001:2022 A.8.9, A.8.32; SOC 2 CC8.1; PCI DSS 6.5.1, 6.5.2; ITIL 4 standard change; COBIT BAI06, BAI10; ISO 9001 8.5.6, 10.2; SLSA; SPDX, CycloneDX; DO-178C and IEC 61508 as vocabulary only) and is not yet articleized, so it cannot be pinned. When it is, pin it in V-b. OWES: a ruling on which of those this project realistically moves toward and which lend words only, banked as a key. Gate: a 📌 line in V-b naming the article, with that list under its OWES.
# - TODO (2026-09-24, the four-day outage; banked the night the power came back): the watchdog that fails the home-hosted sites over has to run on power that is not this apartment's, and the flip has to be one API call from anywhere, because the outage that needs it is the one where every machine in the house is dark. Shape, unridden: a cron on someone else's power (GitHub Actions is free and already trusted with the public repos; a small VPS is the other spelling) curls mikelev.in and npvg.org every few minutes, on K consecutive failures rewrites the A records at the DNS host to a warm standby the publish lane already fills (a gh-pages copy of _site is the cheapest one, at most one publish stale), and flips back on recovery. PRECONDITIONS to settle before designing: a low TTL on both records set NOW, and whether Namecheap's API is even open to this account (it is gated on account size and on a whitelisted caller IP, which rules out the iPhone as the caller); if not, move DNS hosting, not registration, to a provider with an API and health-checked failover. The laptop battery covered every blip until this one; the Z640 UPS (2026-07-18) is a different failure and stays its own line.
# - TODO (2026-09-24, the second brain went dark with the house): the bare repos that serve as remotes for trimnoir and the private journals live on an internal drive of a machine on the same circuit as everything else, so a git server at home would have gone dark the same night. What survives an outage is a copy that leaves the house: `git bundle create trimnoir.bundle --all` writes the whole history as ONE file that rsyncs anywhere and clones back with `git clone trimnoir.bundle`; a nightly bundle onto an external SSD (an SSD has a controller, wear levelling and a rated endurance in the hundreds of terabytes, and a daily bundle will not dent it in a decade; the keychain thumb drive is the same NAND with none of that) plus a mirror on the Mac, the one machine that leaves the house on its own battery. Decide bundle-to-SSD versus a real server AFTER the first bundle exists, not before.
...
# - TODO (2026-09-22, the census is a snapshot and the queue is gitignored): data/botify_census/projects-20260922T214646Z.queue.jsonl holds 10,887 lines of id, org and project for every Web Property project, 0 unparseable, 0 with an extra slash. It lives under data/, so it is on one Mac, in no commit and no payload, and it already drifts: 10,880 rows on 09-21 became 10,887 on 09-22. A walker needs a staleness check and a re-pull cadence, and the file needs a second home (THE LOCAL GIT REMOTE line owns that half). Gate: a walker that refuses a queue older than N days.
# - TODO (2026-09-22, the cookie stopwatch is landed and unwitnessed): commit 7a10797 wraps _admin_cookies in a stopwatch, and no census has run since, so "cookie harvest finished in Ns" has never printed and the six-minute browser launch is still an inference from subtraction. Gate: one census run printing both clocks.
# - TODO (2026-09-22, the mini-rules payload is still unlocated): botify_config returns has_speedworkers but not pap_mini_rules, so per-project config-at-rest has no known endpoint. Candidates, unpriced: PocketRender's own backend calls (a %URL wire sniff would name them), or the project page's Import/Export JSON panel. Gate: one project's mini rules read with no browser in the loop.
# - TODO (2026-09-22, THE LOCAL GIT REMOTE IS A SINGLE POINT OF FAILURE; convicted by a four-day blackout): the trimnoir book ore and every local-only repo live on a filesystem remote mounted on a machine that lost power Monday and stays dark until Thursday, so the 2nd brain, the Jekyll mirror and npvg.org went down together and none of them clone from anywhere else. Two candidate cures, neither chosen: a pinned NixOS git server, so the remote is a SERVICE with an address rather than a path; or keep the local-filesystem remote and systemd-sync it to an EXTERNAL drive that unplugs and travels. The second is cheaper and half-built already, and its gate is that the thumbdrive is actually IN, because a sync to another internal SSD is a backup you cannot carry to an internet cafe. Gate for either: git clone the ore from a second machine with the primary powered off.
# - TODO (2026-09-22, the fail-over that is not a repo): a blackout takes the home-hosted static sites down with the machine, and the cure is registrar-side or GitHub Pages, never a public repo, because the git HISTORY is the private thing. Only built HTML and Markdown release files go to a Pages fail-over; the ore never does. Worth changing registrars for if Namecheap cannot do it. Gate: the site answers from the fail-over with the home machine off.
# - TODO (2026-09-22, the Grimoire is unsanitized ore): the unstoppability material is accumulating in the Grimoire and BotifyML, both proprietary. Scour both for passages that sanitize cleanly into the public Trimnoir book ore for Future Proofing Yourself in the Age of AI. Gate: named articles, not a resolution.
...
# § THE VHOST CENSUS COUNTED THE WRONG NOUN (banked 2026-09-06) -- For any configuration census, enumerate the actual objects the question names and derive the count from that list; never count a nearby syntax token, because comments and multiple attribute paths can make one object look like many.
...
# THE ACTUATOR IS A CAR: anything that runs against a server (--check, tools/list, tools/call, --refresh) rides in PATCHES as its own numbered car, after the patches it depends on, never previewed above them. Sibling of ONE TIMELINE, NO PREVIEWS (2026-08-30; its body moved here from apply.py on 2026-09-26).
...
#   SPECIMEN 2026-09-26 (pipulate, same m alias): 017b9c96 ADDED 366 lines
#   to foo_files.py and nothing else (+366/-0, the constitution moved in
#   from apply.py) under "chore: Remove obsolete comments and
#   documentation", the verb of the ride's other car; that car, 67bdedc8
#   (+0/-365), got "Remove redundant comments and improve clarity in
#   apply.py", and cdd7689c got "Add prose-only growth checks for apply.py",
#   two true subjects in one ride beside one that read the sign backwards.
...
# scripts/two_arm.py  # [4,666 tokens | 17,790 bytes]
...
# tools/connector_tools.py  # [1,031 tokens | 4,211 bytes]
