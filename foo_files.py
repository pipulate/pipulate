...
# - EARMARK: DECLARATION -> MATERIALIZATION (PENDING 2026-09-24): connector auth shape moves to a top-level literal AUTH_SLOT beside the connector that consumes it; wallet.py reads that literal by AST, never imports the connector, and warm/login may materialize a missing connectors.json entry while existing entries always win. connectors.json becomes implementation state, not a JSON authoring exercise for normal users. WITNESS OWED: a scratch wallet containing a non-object _meta entry gains the Botify slot through `wallet.py warm botify` while preserving _meta, with no credential value written.
