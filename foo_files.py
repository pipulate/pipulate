...
# --- START STATS ---
# There are 1,484 already-written articles about this repo at MikeLev.in (Public)
# Velocity: 21 published in the last 7 days
# Markdown negotiated: 2,642 reads (0.21% of all responses)
# DOM hydration: 2970 trapdoor triggers from 15 non-local IPs (top-N sample, self excluded)
# Honeybot telemetry fetched 2026-09-19T08:58Z
# --- END STATS ---
...
# - TODO (2026-09-18, cpr checklist frame): a page question compiled through cpr inherits PromptBuilder._generate_ai_checklist(), so the answer is framed as a five-car code-edit train even when the requested act is only to answer from the walk preview. Next ride: make the checklist frame invocation-selectable and wire only cpr/WALK_CHOP to the lean frame; ahc, default and every existing compile keep the full checklist. The falsifying straddle must show the full checklist absent from a cpr-shaped payload while the same checklist remains present on an ahc/default control. Do not choose by prompt text; the caller names the lane. MECHANISM LANDED 2026-09-19 (--frame lean, passed by cpr; PENDING): delete this line when the compile after a cpr smoke reads the sealed prompt.md at 0 trains and the in-process control still reads train=True.
